...
public class ExperienceResource {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ExperienceDTO> getExperience(@PathVariable(name = "id") final UUID id) {
        return ResponseEntity.ok(experienceService.get(id));
    }

    @PostMapping
    @ApiResponse(responseCode = "201")
    public ResponseEntity<UUID> createExperience(
            @RequestBody @Valid final ExperienceDTO experienceDTO) {
        final UUID createdId = experienceService.create(experienceDTO);
        return new ResponseEntity<>(createdId, HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<UUID> updateExperience(@PathVariable(name = "id") final UUID id,
            @RequestBody @Valid final ExperienceDTO experienceDTO) {
    ...
    public ResponseEntity<Void> deleteExperience(@PathVariable(name = "id") final UUID id) {
